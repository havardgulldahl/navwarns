...
#
# Sample headers (long form from Murmansk, short form from Arkhangelsk):
#  ПРИП МУРМАНСК 291/25 КАРТА 13004 МОТОВСКИЙ ЗАЛИВ ГУБА ЭЙНА
#  ПРИП АРХАНГЕЛЬСК 11/26 Карта 16007
#  ПРИП АРХАНГЕЛЬСК 6/26 карта16022          (no space before chart number)
#  ПРИП АРХАНГЕЛЬСК 3/26 книга 3030          (uses книга instead of карта)

PRIP_HEADER = re.compile(
    r"ПРИП (МУРМАНСК|АРХАНГЕЛЬСК|ЗАПАД) (\d+)/(\d{2})\s*(?:КАРТЫ|КАРТА|КНИГА|КНИГИ)?\s*(\d+(?:\s+\d+)*)?(?:\s+(.+))?",
    re.IGNORECASE,
)
